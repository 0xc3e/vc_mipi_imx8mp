...
patch_bootfs() {
        ...
        cp $KERNEL_SOURCE/arch/arm64/boot/dts/freescale/imx8mp-verdin-nonwifi-dahlia.dtb $WORKING_DIR/build/$TEZI/$BOOTFS/imx8mp-verdin-wifi-dev.dtb
        echo "fdt_overlays=verdin-imx8mp_lt8912_overlay.dtbo" > overlays.txt
...
patch_image_json() {
        cd $BUILD_DIR/$TEZI
        sed -i 's/"description": "Image for BSP verification with QT and multimedia features"/"description": "Vision Components MIPI CSI-2 driver for Toradex Verdin i.MX8MP"/g' image.json
        sed -i 's/"name": "Toradex Embedded Linux Reference Multimedia Image"/"name": "Vision Components MIPI CSI-2 driver for Toradex Verdin i.MX8MP"/g' image.json
}
...
copy_tezi() {
        if [[ ! -z ${media} ]]; then
                ...
                cp -rf $WORKING_DIR/build/$TEZI/* ${media}
                umount ${media}        
        fi
}
